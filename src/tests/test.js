...
const timelesswallet = window.timelesswallet;
...
describe("TimelessWeb3Provider constructor tests", () => {
  test("test constructor.name", () => {
    const provider = new timelesswallet.Provider({ ethereum: {} });
    const web3 = new Web3(provider);
    expect(web3.currentProvider.constructor.name).toBe("TimelessWeb3Provider");
  });

  test("test Ethereum setAddress", () => {
    const provider = new timelesswallet.Provider({
      ethereum: {
        chainId: 1,
        rpcUrl: "",
        isMetaMask: false,
  ...
  test("test Solana setAddress", () => {
    const provider = new timelesswallet.SolanaProvider({
      solana: {
  ...
  test("test setConfig", (done) => {
    const provider = new timelesswallet.Provider({ ethereum: ropsten });
    const web3 = new Web3(provider);
  ...
  test("test eth_chainId", (done) => {
    const provider = new timelesswallet.Provider({ ethereum: bsc });
    const web3 = new Web3(provider);
  ...
  test("test eth_accounts", (done) => {
    const provider = new timelesswallet.Provider({ ethereum: mainnet });
    const web3 = new Web3(provider);
  ...
  test("test eth_sign", (done) => {
    ...
    timelesswallet.postMessage = (message) => {
